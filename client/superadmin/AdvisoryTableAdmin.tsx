...
interface AdvisoryTableAdminProps {
  refreshKey: number;
  onStatusChange: () => void;
}

export default function AdvisoryTableAdmin({ refreshKey, onStatusChange }: AdvisoryTableAdminProps) {
  ...
  // Fetch all advisory applications
  const fetchApplications = async () => {
    try {
      console.log('📥 Fetching advisory applications from dedicated collection...');
      setLoading(true);

      // Use the new advisory-specific endpoint
      ...
      if (data.success) {
        ...
        console.log(`✅ Advisory applications loaded: ${transformedData.length} (from advisory_applications collection)`);
  ...
  const updateStatus = async (applicationId: string, newStatus: string) => {
    try {
      console.log(`🖱️ ${newStatus === 'approved' ? 'Approving' : 'Rejecting'} advisory application:`, applicationId);
      ...
      if (result.success) {
        ...
        console.log(`✅ Advisory application ${applicationId} status updated to: ${newStatus}`);
        
        // Notify parent to refresh stats
        onStatusChange();

        // Show success message
        alert(`✅ Advisory application ${newStatus === 'approved' ? 'approved' : 'rejected'} successfully!`);
      } else {
  ...
  // Get advisory level from position
  const getAdvisoryLevel = (position: string) => {
    const match = position.match(/Level (\d+)/);
  ...
  useEffect(() => {
    ...
    if (levelFilter !== 'all') {
      filtered = filtered.filter(app => 
        getAdvisoryLevel(app.appliedPost || '') === levelFilter
      );
    }

    setFilteredApplications(filtered);
  }, [searchTerm, statusFilter, levelFilter, applications]);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <p className="ml-4 text-amber-700 font-medium">Loading advisory applications...</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col md:flex-row gap-4 bg-amber-50 p-4 rounded-lg border border-amber-200">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder="Search by name, phone, email, or level..."
            value={searchTerm}
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <option value="1">Level 1 (₹25,000)</option>
            <option value="2">Level 2 (₹50,000)</option>
            <option value="3">Level 3 (₹75,000)</option>
            <option value="4">Level 4 (₹1,00,000)</option>
            <option value="5">Level 5 (₹1,50,000)</option>
            <option value="6">Level 6 (₹2,00,000)</option>
          </select>
        ...
        <Button
          ...
          onClick={() => {
            console.log('📊 Exporting advisory applications...');
            const csvContent = "data:text/csv;charset=utf-8," + 
              "Name,Phone,Email,Advisory Level,Location,Status,Fee,Applied Date\n" +
              filteredApplications.map(app => 
                `"${app.name}","${app.phone}","${app.email}","${app.appliedPost}","${getLocationDisplay(app.location)}","${app.application_status || 'pending'}","₹${app.price}","${app.created_at}"`
              ).join("\n");
            
            const encodedUri = encodeURI(csvContent);
            const link = document.createElement("a");
            link.setAttribute("href", encodedUri);
            link.setAttribute("download", `advisory_applications_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        <table className="w-full min-w-[1200px]">
          <thead className="bg-gradient-to-r from-indigo-700 to-purple-700 text-white">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-sm font-semibold">Advisory Level</th>
            ...
            {filteredApplications.length === 0 ? (
              <tr>
                <td colSpan={9} className="px-4 py-8 text-center text-gray-500">
                  No advisory applications found matching your criteria.
                </td>
              </tr>
            ) : (
              filteredApplications.map((app, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="inline-flex items-center gap-2 bg-gradient-to-r from-purple-100 to-indigo-100 px-3 py-1 rounded-full">
                      <span className="font-bold text-indigo-700">{app.appliedPost || 'N/A'}</span>
                    </div>
                    {app.currentDesignation && (
                      <div className="text-xs text-gray-500 mt-1">{app.currentDesignation}</div>
                    )}
                  </td>
          ...
          >
            <div className="bg-gradient-to-r from-indigo-700 to-purple-700 text-white p-6">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-indigo-100 mt-1">Application ID: {selectedApp.application_id}</p>
                </div>
                <button
                  onClick={() => setSelectedApp(null)}
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-semibold text-gray-600">Advisory Level</label>
                  <p className="text-lg font-medium text-indigo-800">{selectedApp.appliedPost}</p>
                </div>
                <div>
                  <label className="text-sm font-semibold text-gray-600">Application Fee</label>
                  <p className="text-lg font-bold text-purple-700">₹{selectedApp.price.toLocaleString()}</p>
                </div>
                <div>
