...
export default function IDCard({ member, onClose }: IDCardProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-start justify-center z-[1000] p-4 pt-8 overflow-y-auto">
      <div className="bg-white rounded-lg max-w-md w-full p-3 my-4">
        ...
        {/* ID Card Design */}
        <div ref={cardRef} className="w-full bg-white" style={{ maxWidth: '600px', margin: '0 auto' }}>
          <div className="flex flex-col md:flex-row" style={{ aspectRatio: '1.586/1' }}>
            {/* Left Side - Black */}
            <div className="w-full md:w-1/2 bg-black text-white flex flex-col">
              {/* Top Section - Logo and Title */}
              <div className="flex-1 flex flex-col items-center justify-center p-3 pb-0">
                <div className="text-center mb-2">
                  ...
                  />
                  <h2 className="text-xs font-bold mb-0.5 leading-tight">LAND DEVELOPERS & OWNERS</h2>
                  <h3 className="text-xs font-semibold">INDIA ASSOCIATION</h3>
                </div>
              </div>

              {/* White Section - We Solve Problems */}
              <div className="bg-white text-black text-center py-1.5">
                <div className="text-xs font-bold">WE SOLVE PROBLEMS OF</div>
              </div>

              {/* Middle Section - Services */}
              <div className="flex-1 flex flex-col items-center justify-center text-center space-y-1 py-2">
                <div className="text-base font-bold">DEVELOPERS</div>
                <div className="text-base font-bold">LAND OWNERS</div>
                <div className="text-base font-bold">REAL ESTATE AGENTS</div>
              </div>

              {/* Red Badge */}
              <div className="px-3 pb-2">
                <div className="bg-white text-black px-2 py-1.5 rounded text-center">
                  <div className="bg-red-600 text-white font-bold text-[9px] py-1 px-2 rounded mb-1">EACH AREA HEAD HAVE 25 POST</div>
                  <p className="font-medium text-[9px]">We Are Appointing Sole Head</p>
                  <p className="text-[8px]">for India, Zone, State, Division,</p>
                  <p className="text-[8px]">District, Tehsil, Pincode, Village</p>
                </div>
              </div>


              {/* Bottom White Section - Contact Info */}
              <div className="bg-white text-black text-center py-1.5 border-t-2 border-gray-300">
                <div className="text-xs font-bold">Mob: 9833752025</div>
                <div className="text-[10px]">Web: instantlly.com</div>
              </div>
            </div>

            {/* Right Side - Red */}
            <div className="w-full md:w-1/2 bg-red-600 text-white p-3 flex flex-col">
              ...
              {/* Member Photo */}
              <div className="bg-white w-full h-32 mb-2 flex items-center justify-center">
                {member.photo ? (
                  <img
                    src={member.photo}
                    alt={member.name}
              ...
              <div className="space-y-1.5 flex-1">
                <div>
                  <div className="text-sm font-bold mb-0.5">Name: {member.name || 'N/A'}</div>
                </div>

                <div>
                  <div className="text-sm font-bold mb-0.5">Mob: {member.phone || 'N/A'}</div>
                  
                </div>

                <div>
                  <div className="text-sm font-bold mb-1">Area Head For</div>

                  {/* Location Fields with Highlighting */}
                  <div className="space-y-0.5">
                    <div className={`px-2 py-1 rounded text-[10px] ${getFieldStyle('country')}`}>
                      <span className="font-semibold">Country:</span> {member.location?.country || 'India'}
                    </div>

                    <div className={`px-2 py-1 rounded text-[10px] ${getFieldStyle('zone')}`}>
                      <span className="font-semibold">Zone:</span> {member.location?.zone || '-'}
                    </div>

                    <div className={`px-2 py-1 rounded text-[10px] ${getFieldStyle('state')}`}>
                      <span className="font-semibold">State:</span> {member.location?.state || '-'}
                    </div>

                    <div className={`px-2 py-1 rounded text-[10px] ${getFieldStyle('division')}`}>
                      <span className="font-semibold">Division:</span> {member.location?.division || '-'}
                    </div>

                    <div className={`px-2 py-1 rounded text-[10px] ${getFieldStyle('district')}`}>
                      <span className="font-semibold">District:</span> {member.location?.district || '-'}
                    </div>

                    <div className={`px-2 py-1 rounded text-[10px] ${getFieldStyle('tehsil')}`}>
                      <span className="font-semibold">Taluka:</span> {member.location?.tehsil || '-'}
                    </div>

                    <div className={`px-2 py-1 rounded text-[10px] ${getFieldStyle('pincode')}`}>
                      <span className="font-semibold">Pincode:</span> {member.location?.pincode || '-'}
                    </div>

                    <div className={`px-2 py-1 rounded text-[10px] ${getFieldStyle('village')}`}>
